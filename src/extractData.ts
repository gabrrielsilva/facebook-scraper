...
export async function extractData(page: Page, keywords: string[], existingData: { id: string }[], groupUrl: string) {
  ...
  const data = await page.evaluate(async (keywords, existingData, groupUrl) => {    
    const ads: { seller: string, description: string, linkToAd: string }[] = [];
    ...
    for await (const post of posts) {
      const linkToProfile = document.querySelector('span > a.x1i10hfl.xjbqb8w.x6umtig.x1b1mbwd.xaqea5y.xav7gou.x9f619.x1ypdohk.xt0psk2.xe8uvvx.xdj266r.x11i5rnm.xat24cr.x1mh8g0r.xexx8yu.x4uap5.x18d9i69.xkhd6sd.x16tdsg8.x1hl2dhg.xggy1nq.x1a2a7pz.xt0b8zv.xzsf02u.x1s688f') as HTMLAnchorElement;
      ...
      for await (const keyword of keywords) {
        ...
        if (
          ...
          ads.push({ seller: linkToProfile.href || 'Não capturado', description: description?.innerText || 'Sem descrição', linkToAd: (<string>linkToAd === groupUrl.split('?')[0] ? linkToAd2 : linkToAd) || groupUrl });
        }
      }
    }
    return ads;
